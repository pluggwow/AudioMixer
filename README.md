...
Правая кнопка на строке → «Закрепить». Закреплённое приложение остаётся в
списке после закрытия — бесцветной строкой (`saturation(0)`), которой можно
заранее выставить громкость: она сохранится и применится при запуске.
