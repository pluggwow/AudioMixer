...
struct StoredAppSettings: Codable, Equatable {
    ...
    /// Есть ли в записи хоть что-то, отличное от настроек по умолчанию.
    ///
    /// Записи заводятся не только когда пользователь что-томенял: достаточно,
    /// например, закрепить строку и открепить обратно. Такие пустышки в списке
    /// «Приложения» только мешают — по ним нечего вспоминать и нечего забывать.
